...
class Alliance(commands.Cog):
    ...
    async def settings(self, interaction: discord.Interaction):
        try:
            ...
            logger.exception("Settings command raised an exception")
            # Preserve the original behavior for certain socket/interaction errors
            if not any(error_code in str(e) for error_code in ["10062", "40060"]):
                # Also print short message for backwards-compat
                print(f"Settings command error: {e}")
            error_message = "An error occurred while processing your request."
            # Multiple fallbacks when interaction replies fail (expired/unknown interaction)
            sent = False
            try:
                ...
                # Try followup if primary response failed (interaction may be unknown)
                try:
                    await interaction.followup.send(error_message, ephemeral=True)
                    sent = True
                except Exception:
                    # As a last resort, try to DM the user (non-ephemeral)
                    try:
                        if getattr(interaction, 'user', None):
                            await interaction.user.send(error_message)
                            sent = True
                    except Exception:
                        logger.exception("Failed all attempts to notify user about settings error")
...
async def setup(bot):
    ...
    # Ensure the class-level app command (settings) is explicitly added to the tree.
    # Some discord.py versions do not automatically bind class-decorated app_commands
    # to the bot's tree when the cog is added, so add it explicitly for reliability.
    try:
        from discord import app_commands
        cmd = getattr(cog, 'settings', None)
        if isinstance(cmd, app_commands.Command):
            try:
                # Avoid double-registration: if a command named the same
                # already exists in the tree (for the same scope), skip adding.
                existing = []
                try:
                    if hasattr(bot.tree, 'get_commands'):
                        existing = list(bot.tree.get_commands())
                    else:
                        existing = list(bot.tree.walk_commands())
                except Exception:
                    try:
                        existing = list(bot.tree.walk_commands())
                    except Exception:
                ...
                # Helper to check if a matching command exists
                def _has_matching(cmd_name, guild_id=None):
                    for ec in existing:
                        try:
                            if getattr(ec, 'name', None) != cmd_name:
                                continue
                            # Some command objects have .guild_id attribute
                            egid = getattr(ec, 'guild_id', None)
                            if guild_id:
                                # looking for guild-scoped add
                                if egid and int(egid) == int(guild_id):
                                    return True
                            else:
                                # looking for global add: prefer commands without a guild_id
                                if not egid:
                                    return True
                        except Exception:
                            continue
                ...
                if gid:
                    if _has_matching(cmd.name, guild_id=gid):
                        print(f"Skipping add: /{cmd.name} already registered for guild {gid}")
                    else:
                        try:
                            bot.tree.add_command(cmd, guild=discord.Object(id=int(gid)))
                            print(f"Added /{cmd.name} to tree for guild {gid}")
                        except Exception:
                            # fallback to global add if guild add failed
                            if _has_matching(cmd.name, guild_id=None):
                                print(f"Skipping add: /{cmd.name} already registered globally (fallback)")
                            else:
                                bot.tree.add_command(cmd)
                                print(f"Added /{cmd.name} to global tree (fallback)")
                else:
                    if _has_matching(cmd.name, guild_id=None):
                        print(f"Skipping add: /{cmd.name} already registered globally")
                    else:
                        bot.tree.add_command(cmd)
                        print(f"Added /{cmd.name} to global tree")
            except Exception as e:
                print(f"Failed to add /settings command to tree: {e}")
    except Exception:
        pass

    # Add a small admin-only debug command to list currently-registered app commands.
    try:
        from discord import app_commands

        async def _debug_list_commands(interaction: discord.Interaction):
            try:
        ...
        cmd = app_commands.Command(_debug_list_commands, name='debug_list_commands', description='(Admin) List registered app commands')
        # Add to guild if dev GUILD_ID exists otherwise global
        import os
        gid = os.getenv('GUILD_ID')
        try:
            if gid:
        ...
        # non-fatal: debug command best-effort
        pass
