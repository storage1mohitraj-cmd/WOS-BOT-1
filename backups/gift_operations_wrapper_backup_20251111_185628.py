"""Compatibility wrapper for the minimal gift operations cog.

This module exists so code that imports `cogs.gift_operations` keeps working
while we iterate. It delegates to `cogs.gift_operations_minimal`.
"""

try:
    from .gift_operations_minimal import GiftOperations, setup as _minimal_setup
except Exception:
    # If something goes wrong importing the minimal implementation, raise a clear error
    raise


async def setup(bot):
    """Load the minimal GiftOperations cog."""
    await _minimal_setup(bot)
        self.cog = cog
...
class GiftOperations(commands.Cog):
    ...
    async def _process_queue_item(self, queue_item):
        ...
        if message and channel:
            ...
            processing_message = await channel.send(embed=processing_message)
...
class GiftView(discord.ui.View):
    ...
    async def delete_gift_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            self.cog.logger.exception(f"Error in use_gift_alliance_button: {str(e)}")
            await interaction.response.send_message("❌ An error occurred while processing the request.", ephemeral=True)

    @discord.ui.button(
        label="Redeem Latest (Alliance)",
        emoji="🔁",
        style=discord.ButtonStyle.success,
        custom_id="redeem_latest_alliance",
        row=1
    )
    async def redeem_latest_alliance_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Quick flow: pick an alliance and queue redemption of the latest active gift code."""
        try:
            ...
            if not available_alliances:
                await interaction.response.send_message(embed=discord.Embed(title="❌ No Available Alliances", description="You don't have access to any alliances.", color=discord.Color.red()), ephemeral=True)
                return
            ...
            embed = discord.Embed(title="🔁 Redeem Latest Gift Code", description=("Select an alliance to redeem the latest active gift code for:\n\n" "**Alliance List**\n" "━━━━━━━━━━━━━━━━━━━━━━\n"), color=discord.Color.green())
            view = AllianceSelectView(alliances_with_counts, self.cog)
            view.current_select.options.insert(0, discord.SelectOption(label="ALL ALLIANCES", value="all", description=f"Apply to all {len(alliances_with_counts)} alliances", emoji="🌐"))
            ...
            async def alliance_callback(select_interaction: discord.Interaction):
                try:
                    ...
                    self.cog.cursor.execute("""
                        SELECT giftcode FROM gift_codes
                    ...
                    row = self.cog.cursor.fetchone()
                    if not row:
                        await select_interaction.response.edit_message(content="No active gift codes available.", embed=None, view=None)
                        return

                    selected_code = row[0]
                    await self.cog.add_manual_redemption_to_queue(selected_code, all_alliances, select_interaction)
                    queue_status = await self.cog.get_queue_status()
                    alliance_names = [next((n for a_id, n, _ in alliances_with_counts if a_id == aid), 'Unknown') for aid in all_alliances[:3]]
                    ...
                    queue_embed = discord.Embed(title="✅ Redemptions Queued Successfully", description=(f"Gift code redemptions added to the queue.\n\n" f"**Your Redemption**\n" f"━━━━━━━━━━━━━━━━━━━━━━\n" f"🎁 **Gift Code:** `{selected_code}`\n" f"🏰 **Alliances:** {alliance_list}\n" f"━━━━━━━━━━━━━━━━━━━━━━\n\n" f"**Full Queue Details**\n" f"{queue_info}\n\n" f"📊 **Total items in queue:** {queue_status['queue_length']}\n" f"📍 **Your position:** #{your_position if your_position else 'Processing'}\n\n" f"💡 You'll receive notifications as each alliance is processed."), color=discord.Color.green())
                    queue_embed.set_footer(text="Gift codes are processed sequentially to prevent issues.")
                    await select_interaction.response.edit_message(embed=queue_embed, view=None)
                except Exception as e:
                    self.cog.logger.exception(f"Error in redeem-latest alliance flow: {e}")
                    if not select_interaction.response.is_done():
                        await select_interaction.response.send_message("❌ An error occurred while processing your selection.", ephemeral=True)
                    else:
                        await select_interaction.followup.send("❌ An error occurred while processing your selection.", ephemeral=True)
            ...
            try:
                ...
                self.cog.logger.exception(f"Failed to send alliance selector for redeem-latest: {e}")
                try:
                    await interaction.response.send_message("Failed to open alliance selector.", ephemeral=True)
                except Exception:
                    pass

        except Exception as e:
            self.cog.logger.exception(f"redeem_latest_alliance_button error: {e}")
            try:
                await interaction.response.send_message("❌ An unexpected error occurred.", ephemeral=True)
            except Exception:
                pass
...
class GiftView(discord.ui.View):
    ...
    async def redeem_latest_alliance_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # Build selector view
            embed = discord.Embed(
                title="🔁 Redeem Latest Gift Code",
                description=(
                    "Select an alliance to redeem the latest active gift code for:\n\n"
                    "**Alliance List**\n"
                    "━━━━━━━━━━━━━━━━━━━━━━\n"
                ),
                color=discord.Color.green()
            ...
            async def alliance_callback(select_interaction: discord.Interaction):
                try:
                    ...
                    # Queue the redemption(s)
                    await self.cog.add_manual_redemption_to_queue(selected_code, all_alliances, select_interaction)

                    queue_status = await self.cog.get_queue_status()

                    alliance_names = []
                    for aid in all_alliances[:3]:
                        name = next((n for a_id, n, _ in alliances_with_counts if a_id == aid), 'Unknown')
                        alliance_names.append(name)
            ...
            # Send or edit the message depending on response state
            try:
                if not interaction.response.is_done():
                    await interaction.response.edit_message(embed=embed, view=view)
                else:
