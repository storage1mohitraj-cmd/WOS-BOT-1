...
# Defer importing heavy/optional pymongo-related modules so that the module
# can still be imported in environments where `pymongo` isn't installed
# (for example, during partial deploys where dependency installation failed).
...
# Avoid importing `mongo_client_wrapper` at module import time because it
# itself imports `pymongo`. Instead, import `get_mongo_client` lazily inside
# the class initializer so that the module can be imported even when
# `pymongo` installation failed; instantiation will then raise a clear error.
...
class ReminderStorageMongo:
    ...
    def __init__(self, client=None):
        if not _PYMONGO_AVAILABLE:
            raise RuntimeError(
                'pymongo (and/or bson) is not available in this Python environment. '
                'Install dependencies (ensure `pymongo` is in requirements) and restart the bot.'
            )
        # Lazy import to avoid import-time dependency on pymongo in mongo_client_wrapper
        if client is None:
